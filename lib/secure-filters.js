...
// Matches alphanum plus ",._-".
// ESAPI doesn't consider "-" safe, but we do. It's both URI and HTML safe.
// The attack where an attempt to open an HTML comment won't work because
// "(lessthan)--" is turned into "\x3C--".
var JS_NOT_WHITELISTED        = /[^,\-\.0-9A-Z_a-z]/g;
var JS_NOT_WHITELISTED_SINGLE = /[^,\-\.0-9A-Z_a-z]/;
...
function jsObjScrub(obj) {
  var key;

  var safeKeys = true;
  for (key in obj) {
    // only own-properties get serialized
    if (!obj.hasOwnProperty(key)) {
      continue;
    }
    var hasUnsafe = JS_NOT_WHITELISTED_SINGLE.test(key);
    if (hasUnsafe) {
      safeKeys = false;
      break;
    }
  ...
  if (safeKeys) {
    // JSON will recurse into the object to visit the values
    return obj;
  }
  ...
  for (key in obj) {
    ...
    var safeK = key.replace(JS_NOT_WHITELISTED, jsSlashEncoder);
    replacement[safeK] = obj[key];
  }

  return replacement;
}

function jsObjStringFilter(_ignored, value) {
  switch(typeof value) {
  case 'object':
    ...
    return jsObjScrub(value);

  case 'string':
    return value.replace(JS_NOT_WHITELISTED, jsSlashEncoder);
...
secureFilters.jsObj = function(val) {
  var result = JSON.stringify(val, jsObjStringFilter);

  // since all backslashes just got escaped to "\x5C" remove the redundant JSON
  // built-in escaping:
