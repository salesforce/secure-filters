...
/**
 * Adds this module's filters to handlebars as helpers.
 *
 * **USAGE**:
 *
 * ```js
 *   var secureFilters = require('secureFilters');
 *   var defaultEscape = true;
 *   var handlebars = secureFilters.configureHandlebars(
 *     require('handlebars'),
 *     secureDefaultEscape
 *   );
 * ```
 ...
 *
 * @param {Object} handlebars the Handlebars package object
 * @param {boolean} [defaultEscape=true] replace the default handlebars escape function
 * @return {Object} the same Handlebars package object
 */
secureFilters.configureHandlebars = function(handlebars, defaultEscape) {
  if (defaultEscape === null || defaultEscape === undefined) {
    defaultEscape = true;
  }

  var SafeString = handlebars.SafeString;

  function makeHelper(filterName) {
    var filter = secureFilters[filterName];
    return function(value) {
      return new SafeString(filter(value));
    };
  }

  for (var i = 0; i < TO_CONFIGURE.length; i++) {
    var filterName = TO_CONFIGURE[i];
    handlebars.registerHelper(filterName, makeHelper(filterName));
  }

  // replace default handler
  if (defaultEscape) {
    var html = secureFilters.html;
